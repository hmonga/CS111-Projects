...
public class ISBN {
    ...
    public static int[] calculateISBN13(int[] shortIsbn) {
        ...
        bigIsbn[2] = 8; 
        for(int i = 3; i < 13; i++){
            bigIsbn[i] = shortIsbn[j];
            j++;
        }

        //multiply with 1 & 3 alternatly 
        for(int i = 0; i < 12; i++){
            if(i % 2 == 0){
               sumOFProducts += bigIsbn[i] * 1; 
            }
            else{
               sumOFProducts += bigIsbn[i] * 3; 
            }
        }
