public class Communities {
    ...
    public static double findAverageLifeExpectancy(double[][] data) {
        ...
        return sum / counter; // replace this line
    }
    ...
    public static int numberOfCommunities(double[][] data, int columnIndex, double percentageThreshold,
        ...
        if (isDisadvantage == true) {
            ...
            for (int i = 0; i < data.length; i++) {
                if (data[i][21] == 1) {
                    if ((data[i][columnIndex] * 100) >= percentageThreshold) {
                        counter++;
                    }
                }
            }
            ...
            int counter = 0;
            for (int i = 0; i < data.length; i++) {
                if ((data[i][columnIndex] * 100) >= percentageThreshold && data[i][21] == 0) {
                    counter++;
                }
            }
            return counter;
    ...
    public static double largestRacialDemographic(double[][] data, int community) {
        ...
        for (int i = 0; i < data.length; i++) {
            for (int j = 0; j < 8; j++) {
                if (max < data[community][j]) {
                    max = data[community][j];
                }
            }
        }
    ...
    public static int mostPopulatedDisadvantagedWithLowIncome(double[][] data) {
        ...
        for (int i = 0; i < data.length; i++) {
            for (int j = 0; j < data[i].length; j++) {
                if ((data[i][21] == 1) && (data[i][20] == 1) && data[i][11] > pop) {
                    rowIndex = i;
                    pop = data[i][11];
                }
            }
        }
        return rowIndex; // replace this line
    }
